...
type PreUrl struct {
	Url    string `json:"datos"` //Deminimos que el campo Url será de tipo igual que el item "datos" de JSON obtenido
	Client *http.Client
}
...
type Diaria struct { //Obtenemos los diferentes datos correspondientes a la respuesta JSON y las asignamos a cada uno de los campos
	...
	Time             time.Time `json:"-"` //No tiene vinculación con el JSON
}

// Realizaremos una funciómn para gestionar la obtención de los datos climatológicos y el primero que tenemos que hacer es hacer referencia al package nativo http a través del struct Prediccio
...
func GetPrediccio(url string) ([]int, error) {
	...
	var precipitacio, tempMax, tempMin, humitad int
	...
	//parte := []int{precipitacion, tempMax, tempMin, humedad}  REVISAR!!!!!!
	return &parte, err
