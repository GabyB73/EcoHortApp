...
import
	...
	func (repo *SQLiteRepository) InsertRegistre(registre Registres) (*Registres, error) {
		...
		res, err := repo.Conn.Exec(sentencia, registre.Data.Unix, registre.Precipitacio, registre.TempMaxima, registre.TempMinima, registre.Humitat)
		if err != nil {
			return nil, err
		}
		...
		return &registre, nil //Preparar el retornocon un objeto o nil en caso de error
	}
	...
	func (repo *SQLiteRepository) LlegirTotsRegistres() ([]Registres, error) {
		...
		func (repo *SQLiteRepository) ActualitzarRegistre(id int64, actualitzar Registres) error{
			...
			if id == 0{
				return error.New("La ID a actualizar es incorrecta")
			}
		...
		func(repo *SQLiteRepository) BorrarRegistre(id int64) error {
			res, err := repo.Conn.Exe("delete from registres where id = ?", id)
			if err != nil {
				return err
			}
